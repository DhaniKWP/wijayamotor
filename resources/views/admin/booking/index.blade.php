                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <span class="inline-flex items-center px-2 py-0.5 mt-2 rounded text-[10px] font-bold bg-slate-100 text-slate-600">
                            {{ strtoupper($booking->tipe_booking ?? 'BENGKEL') }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        @elseif($booking->status == 'confirmed')
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700">Disetujui</span>
                        @elseif($booking->status == 'process')
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-purple-100 text-purple-700">Dikerjakan</span>
                        ...
                        @elseif($booking->status == 'cancelled')
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-700">Ditolak</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                        <button type="button" data-id="{{ $booking->id }}" onclick="openDetailModal(this.getAttribute('data-id'))" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-2 rounded-lg text-xs font-bold transition inline-flex items-center">Detail</button>   
                        @if($booking->status == 'pending')
...
<div id="modalDetail" class="fixed inset-0 z-50 hidden bg-slate-900/50 backdrop-blur-sm overflow-y-auto h-full w-full flex items-center justify-center">
    <div class="relative w-full max-w-2xl mx-4 my-8 bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <ul class="space-y-2 text-sm">
                        ...
                        <li><span class="text-slate-500 block text-xs">No. Telepon:</span> <span class="font-medium text-slate-800" id="modalPhone">-</span></li>
                    </ul>
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <ul class="space-y-2 text-sm">
                        ...
                        <li><span class="text-slate-500 block text-xs">Kilometer:</span> <span class="font-medium text-slate-800" id="modalKm">-</span></li>
        ...
        <div class="bg-slate-50 px-6 py-4 border-t border-slate-100 flex justify-end">
            <button onclick="closeModal()" class="px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-lg text-sm font-bold hover:bg-slate-50 transition shadow-sm">Tutup</button>
        </div>
    </div>
</div>
