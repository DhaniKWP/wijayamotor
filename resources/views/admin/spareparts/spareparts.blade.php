...
<head>
    ...
    <title>Kelola Sparepart - Wijaya Motor</title>
...
<body class="bg-[#F8FAFC] flex h-screen overflow-hidden antialiased" x-data="{ sidebarOpen: false, showAddModal: {{ $errors->any() ? 'true' : 'false' }}, showEditModal: false, editAction: '', editName: '', editPrice: '', editStock: '', editDesc: '' }">
    ...
    <aside class="w-64 bg-primary text-white flex flex-col hidden md:flex shrink-0">
        ...
        <nav class="flex-1 px-4 py-8 space-y-2 overflow-y-auto">
            <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3 text-slate-400 hover:bg-white/5 hover:text-white px-4 py-3.5 rounded-xl font-medium transition-all group">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <header class="bg-white h-20 flex items-center justify-between px-8 z-10 shrink-0 border-b border-slate-200/60">
            <h1 class="text-xl font-bold text-slate-800 tracking-tight">Gudang Sparepart</h1>
            <button @click="showAddModal = true" class="bg-primary hover:bg-[#112a4f] text-white px-5 py-2.5 rounded-xl font-semibold flex items-center transition-all text-sm shadow-sm hover:shadow">
        ...
        <main class="flex-1 overflow-y-auto p-8">
            @if(session('success'))
            <div class="bg-emerald-50 border border-emerald-100 text-emerald-600 px-4 py-3 rounded-xl mb-6 flex items-center text-sm font-medium">
            ...
            <div class="bg-white rounded-2xl border border-slate-200/60 overflow-hidden shadow-sm">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-100">
                        <thead>
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider bg-white">Produk</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider bg-white">Harga Jual</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider bg-white">Status Stok</th>
                                <th scope="col" class="px-6 py-4 text-right text-xs font-semibold text-slate-400 uppercase tracking-wider bg-white">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-slate-50">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-slate-700">Rp {{ number_format($item->price, 0, ',', '.') }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button 
                                            @click="showEditModal = true; editAction = '{{ route('admin.spareparts.update', $item->id) }}'; editName = '{{ addslashes($item->name) }}'; editPrice = '{{ intval($item->price) }}'; editStock = '{{ $item->stock }}'; editDesc = '{{ addslashes($item->description) }}';"
                                        ...
                                        <form action="{{ route('admin.spareparts.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin bro mau hapus {{ $item->name }} secara permanen?');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-20 text-center">
                                    ...
                                    <p class="text-sm font-semibold text-slate-600">Gudang masih kosong</p>
    ...
    <div x-show="showAddModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
        <div @click.away="showAddModal = false" class="bg-white w-full max-w-lg rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div class="overflow-y-auto p-6">
                ...
                <form action="{{ route('admin.spareparts.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                    @csrf
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Nama Barang</label>
                        <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Ban Bridgestone R15" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:ring-2 focus:ring-secondary/50 focus:border-secondary outline-none transition-all placeholder-slate-300 text-slate-800">
                    </div>
                    
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Harga Jual (Rp)</label>
                            <input type="number" name="price" value="{{ old('price') }}" required placeholder="Contoh: 850000" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:ring-2 focus:ring-secondary/50 focus:border-secondary outline-none transition-all placeholder-slate-300 text-slate-800">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Stok Awal</label>
                            <input type="number" name="stock" value="{{ old('stock') }}" required placeholder="Contoh: 10" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:ring-2 focus:ring-secondary/50 focus:border-secondary outline-none transition-all placeholder-slate-300 text-slate-800">
                        </div>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Deskripsi Produk</label>
                        <textarea name="description" rows="3" placeholder="Jelaskan spesifikasi barang..." class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:ring-2 focus:ring-secondary/50 focus:border-secondary outline-none transition-all resize-none placeholder-slate-300 text-slate-800">{{ old('description') }}</textarea>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Foto Produk</label>
        ...
        <div @click.away="showEditModal = false" class="bg-white w-full max-w-lg rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div class="overflow-y-auto p-6">
                <form x-bind:action="editAction" method="POST" enctype="multipart/form-data" class="space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Stok Tersedia</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Ganti Foto (Opsional)</label>
                        <div class="border border-dashed border-slate-300 rounded-xl p-4 text-center hover:bg-slate-50 transition-colors">
