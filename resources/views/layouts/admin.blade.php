...
<body class="bg-[#F8FAFC] flex h-screen overflow-hidden antialiased" x-data="{ sidebarOpen: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-30 w-64 bg-primary text-white flex flex-col transition-transform duration-300 md:relative md:translate-x-0 shrink-0 shadow-2xl md:shadow-none">
        ...
        <nav class="flex-1 px-4 py-8 space-y-2 overflow-y-auto">
            ...
            <a href="{{ route('admin.services.index') }}" class="flex items-center space-x-3 px-4 py-3.5 rounded-xl font-medium transition-all {{ request()->routeIs('admin.services.*') ? 'bg-white/5 text-white border-r-4 border-secondary' : 'text-slate-400 hover:bg-white/5 hover:text-white group' }}">
            ...
            <a href="{{ route('admin.spareparts.index') }}" class="flex items-center space-x-3 px-4 py-3.5 rounded-xl font-medium transition-all {{ request()->routeIs('admin.spareparts.*') ? 'bg-white/5 text-white border-r-4 border-secondary' : 'text-slate-400 hover:bg-white/5 hover:text-white group' }}">
