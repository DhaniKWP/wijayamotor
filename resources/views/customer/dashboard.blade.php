...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
    ...
    <main class="lg:col-span-3">
        ...
        @if(empty(Auth::user()->phone) || empty(Auth::user()->address))
            <div class="mb-8 p-5 rounded-xl bg-amber-50 border border-amber-200 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div class="flex items-start gap-3">
                    <div class="bg-amber-100 p-2 rounded-lg text-amber-600 shrink-0">
                        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                    </div>
                    <div>
                        <h3 class="text-amber-800 font-bold text-lg mb-1">Profil Belum Lengkap!</h3>
                        <p class="text-amber-700 text-sm">Anda tidak dapat membuat Booking sebelum melengkapi Nomor HP dan Alamat. Silakan lengkapi data diri Anda.</p>
                    </div>
                </div>
                <a href="{{ route('customer.profile.settings') }}" class="shrink-0 bg-amber-600 hover:bg-amber-700 text-white font-bold text-sm px-6 py-2.5 rounded-lg transition shadow-sm">
                    Lengkapi Sekarang
                </a>
            </div>
        @endif
